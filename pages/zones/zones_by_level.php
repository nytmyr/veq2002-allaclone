...
foreach ($ignore_zones AS $zid) {
    ...
    $v = " AND ";
	$query .= " $v $zones_table.min_status = 0 AND $zones_table.expansion <= 6";
}
...
while ($res = mysqli_fetch_array($result)) {
    ...
    $query = "SELECT $npc_types_table.level
		  ...
		  AND $npc_types_table. merchant_id = 0
		  ";
    if ($hide_invisible_men == TRUE) {
        $query .= "
			...
			AND $npc_types_table.class < 20
		";
    }
    $query .= " AND $npc_types_table.level>$MinimumNpcLvl
				GROUP BY $npc_types_table.id, $spawn2_table.spawngroupID";
...
$print_buffer .= "<thead class='menuh'><th>Zone</th>
				<th class=tab_title>ZEM</th>
				<th class=tab_title>Avg Lvl</th>";
$LevelMax = 0;
for ($i = 0; $i <= ($server_max_npc_level / 5); $i++) {
    $LevelMax += 5;
    $LevelMin = $LevelMax - 4;
