...
if ($name == "potimea" ) {
	...
	$page_title .= "<br><span style='color: purple; font-size: 15px;'><b>First entered by the guild <a href='/charbrowser/index.php?page=guild&guild=" . $guild_name . "'>" . $guild_name . "</a> on " . $achievement_time . "</b>";
}
...
$querytwo = "
	...
	-- AND ((zp.x != 0 AND zp.y != 0 AND zp.z != 0) OR zp.zone LIKE 'poknowledge')
	-- `AND zp.id NOT BETWEEN 1086 AND 1105
	-- `AND zp.id NOT BETWEEN 1334 AND 1347
	-- AND ((zz.zoneidnumber NOT BETWEEN 200 AND 201 AND zp.target_zone_id < 200) OR (zz.zoneidnumber BETWEEN 200 AND 201 AND zp.target_zone_id >= 200))
	-- AND ((zz.zoneidnumber NOT BETWEEN 203 AND 223 AND zp.target_zone_id < 200) OR (zz.zoneidnumber BETWEEN 203 AND 223 AND zp.target_zone_id >= 200))
	AND (CASE
			WHEN (z.zoneidnumber < 200 OR z.zoneidnumber = 407)
				THEN (zp.target_zone_id < 200 OR zp.target_zone_id = 202 OR zp.target_zone_id = 407)
			WHEN z.zoneidnumber BETWEEN 200 AND 201
				THEN zp.target_zone_id >= 200
			WHEN z.zoneidnumber BETWEEN 203 AND 223
				THEN zp.target_zone_id >= 200
			WHEN z.zoneidnumber = 202
				THEN zp.target_zone_id > 0
		END)
	AND zz.zoneidnumber != 109
	AND z.zoneidnumber != 109
";
...
if ($zone["zoneidnumber"] == 39 || $zone["zoneidnumber"] == 89 || $zone["zoneidnumber"] == 105 || $zone["zoneidnumber"] == 108 || $zone["zoneidnumber"] == 128 || $zone["zoneidnumber"] == 158) {
	if ($show_item_as_key_req == true) {
		...
		if ($zone["zoneidnumber"] == 158) {
			$req .= "[<img src='$icons_url\item_1172.png' width='25px' height='25px'/><a href=?a=item&id=22198>The Scepter of Shadows</a>]";
		}
	} else {
		$setcolor = "red";
		$req .= "[Key Required]";
...
if ($zone["flag_needed"] == "" && $zone["min_level"] == 0 && $zone["zoneidnumber"] != 39 && $zone["zoneidnumber"] != 89 && $zone["zoneidnumber"] != 105 && $zone["zoneidnumber"] != 108 && $zone["zoneidnumber"] != 128 && $zone["zoneidnumber"] != 158) {
	$req .= "None";
	$setcolor = "green";
}
...
if ($mode == "npcs") {
	...
	//	";
	$query = "SELECT $npc_types_table.id,$npc_types_table.class,$npc_types_table.level,$npc_types_table.trackable,$npc_types_table.maxlevel,$npc_types_table.race,$npc_types_table.`name`,$npc_types_table.`lastname`,$npc_types_table.maxlevel,$npc_types_table.loottable_id,$npc_types_table.rare_spawn,$npc_types_table.raid_target,$npc_types_table.`hide_npc` 
		FROM $npc_types_table,$zones_table";
    $query .= " WHERE $zones_table.short_name = '$name'
				AND $npc_types_table.`hide_npc` != 1
				AND ((CASE 
						WHEN $zones_table.zoneidnumber = 407 
							THEN CAST(FLOOR($npc_types_table.`id` / 1000) AS INT) = 5 
						ELSE CAST(FLOOR($npc_types_table.`id` / 1000) AS INT) = $zones_table.zoneidnumber
					END)
				OR $npc_types_table.`in_zone_id` = $zones_table.zoneidnumber)				
		";
    ...
    if ($hide_invisible_men == TRUE) {
		...
		$query .= "
			...
			AND $npc_types_table.id NOT BETWEEN 202343 AND 202365
		";
    }
    ...
    if (mysqli_num_rows($result) > 0) {
		...
		$print_buffer .= "<p><b>Bestiary</b><p><table border=0><tr>";
        if ($ZoneDebug == TRUE) {
