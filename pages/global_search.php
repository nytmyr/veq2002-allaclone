...
if($_GET['get_data']){
    ...
    if($_GET['fetch_type'] == "global_tradeskills"){
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            echo '<li><a href="?a=recipe&id=' . $row['id'] . '">' . $row['name'] . ' ' . (trim($tradeskill) ? '(' . $tradeskill . ')' : '') . '</a></li>';
        }
        echo '</ul>';
    }
    ...
    if($_GET['fetch_type'] == "global_zones"){
        $query = "SELECT * FROM `zone` WHERE `short_name` LIKE '%" . $name . "%' OR `long_name` LIKE '%" . $name . "%' AND `min_status` = 0 ORDER BY `long_name`";
        $result = db_mysql_query($query);
        echo '<ul>';
        while ($row = mysqli_fetch_array($result)) {
    ...
    if($_GET['fetch_type'] == "global_mobs"){
        $query = "
			...
			WHERE `name` LIKE '%" . str_replace(' ', '_', $name) . "%' 
			AND ((`race` = 127 AND `mindmg` != 1 AND `maxdmg` != 4 AND `show_name` = 1) OR (`race` != 127))
			AND ((`race` = 240 AND `mindmg` != 1 AND `maxdmg` != 4 AND `show_name` = 1) OR (`race` != 240))
			AND (`mindmg` != 0 AND `maxdmg` != 4)
			AND bodytype NOT BETWEEN 65 AND 67
			AND show_name = 1
			AND hide_npc = 0
			AND untargetable = 0
			ORDER BY `name`
...
$global_search_count = array(
    "zones" => array("Zones", "SELECT COUNT(*) as found_count FROM `zone` WHERE `short_name` LIKE '%" . $name . "%' OR `long_name` LIKE '%" . $name . "%' AND `min_status` = 0"),
    "mobs" => array("Mobs", "
    ...
    "tradeskills" => array("Trade Skills", "
											SELECT COUNT(*) as found_count 
											FROM `tradeskill_recipe`  
											WHERE `name` LIKE '%" . $name . "%'
										"),
